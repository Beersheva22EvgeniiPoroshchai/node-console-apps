...
async updateEmployee(employee) {
    const doc = await this.#collection.updateOne({_id:employee.id}, //filter: empl.id
        {$set:{department:employee.department, salary:employee.salary}});  //update
        return doc.matchedCount == 1 ? employee : null;   //matchedCount: updated employee(1)
}
...
function toDocument(employee) {
    const document = {...employee, _id: employee.id};  //get all employee's field, except _id
    delete document.id;   //delete id field, _id stays
    return document;
}

function toEmployee(document) {   //vise versa
